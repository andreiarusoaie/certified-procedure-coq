...
Require Import langdef.
Require Import sound.

Module LSoundness : Soundness Lang Semantics Derivatives.
  Import Utils.
  Import Lang.
  Import Semantics.
  Import Derivatives.
  ...
  | der :
      forall S', 
        ...
        RL_alpha_equiv_S Semantics.S S' -> disjoint_vars_rules S' g ->
  ...
  Proof.
    ...
    case_eq n.
    - intros n0; subst n.
      unfold complete in C0.
      destruct C0 as (gamma' & H0 & H1).
  ...
  Lemma one_step :
    forall tau rho phi phi',
      ...
      (exists phi1,
         In phi1 (SynDerML phi Semantics.S) /\
  ...
  Proof.
    ...
    - simpl in H.
      ...
      + subst.
        ...
        * simpl; left; trivial.
      + simpl.
        case_eq (RLFormula_eq_dec g' a0); intros.
        subst.
        apply IHG; trivial.
        simpl. right.
        apply IHG; trivial.
  ...
  Lemma der_in_Delta :
    forall phi phi' alpha,
      In alpha Semantics.S ->
      In (phi => phi') G0 ->
      In (SynDerML' phi alpha => phi') (Delta Semantics.S G0).
  Proof.
  ...
  Proof.
    ...
    apply SatML_Exists.
    simpl.
    set (vars := (FreeVars (phi_l :: phi_r :: nil))).
    simpl in *.
  ...
  Lemma finite_sound :
    forall n tau rho phi phi',
      ...
      inF (phi => phi') ->
      completeN tau n ->
      steps (Delta Semantics.S G0) ->
      startsFrom tau rho phi ->
      total ->
      (forall p p', In (p => p') G0 -> SDerivable p) ->
      (forall F, In F G0 -> wfFormula F) ->
      (forall F, In F Semantics.S -> wfFormula F) ->
      (forall g, In g G0 -> disjoint_vars_rules Semantics.S g) ->
      SatRL tau rho (phi => phi').
  Proof.
    intros n.
    ...
    - intros tau rho phi phi' NE WF H' H0 H1 H2 T Ax WFF1 WFF2 D.
      ...
      + (* case 1: the goal in F is in G0 *)
        ...
        apply first_transition with (rho := rho) (phi := phi) (n := n0); trivial.
        (* prepare cover_step *)
        assert (SF : startsFrom tau rho phi); trivial.
        ...
        intros F HF.
        assert (HF' : In F Semantics.S); trivial.
        apply D in H3.
        ...
        apply H with (tau := (Path_i tau 1)) (m := n0); trivial.
        apply wf_subpath; trivial.
        subst phi1.
        apply G_in_F with (G := (Delta Semantics.S G0)); trivial.
      ...
      + (* case 2 : goal is in F, but not in G0 *)
        ...
        * (* goal eliminated by circularity *)
          ...
          simpl in H9.
          apply valid_impl
          with (gamma := gamma) (phi' := (EClos (lhs c0)))
          ...
          destruct H12 as (gamma0 & gamma' & T0 & T1 & H12).
           ...
           inversion H2.
           subst gamma0.
           apply cover_step with (rho := rho') (phi := (lhs c0)) in H12; trivial.
           destruct H12 as (alpha & phic_1 & H12 & H13 & H14).
           
           (* inductive hypothesis for a shorter path (covered by circ) *)
           assert (H17 : SatRL (Path_i tau 1) rho' (phic_1 => (rhs c0))).
           {
             apply H with (m := n0); trivial.
             - apply wf_subpath; trivial.
             - apply G_in_F with (G := (Delta Semantics.S G0)); trivial.
               subst phic_1.
               apply der_in_Delta; trivial.
               destruct c0; simpl; trivial.
             - unfold startsFrom.
               exists gamma'.
               rewrite shift_index.
               simpl.
           ...
           simpl in H17.
           destruct H17 as (n1 & i & gamma_i & H17 & H18 & H19 & H20).
           ...
           assert (H21 : SatML gamma_i rho (SynDerML' (phi) c')).
           {
             ...
             apply RL_alpha_equiv_wf with (F := c0); trivial.
             apply WFF1; trivial.
             destruct c0 as (phic & phic'), c' as (c_ & c_').
             simpl in *.
                                ...
                                ((SynDerML' phi c') => phi')).
           {
             ...
             - unfold complete.
               ...
               split;trivial.
               + rewrite shift_index.
                 assert (H22: (n0 - i + (i + 1)) = Datatypes.S n0).
                 * omega.
                 * rewrite H22; trivial.
             - unfold startsFrom.
               exists gamma_i.
           ...
           simpl in H20'.
           destruct H20' as (n2 & j & gamma_j & H21' & H22 & H23 & H24).
           unfold SatRL.
           split; try simpl; trivial.
           left.
           exists (Datatypes.S n0), (j + (i + 1)), gamma_j.
           split; trivial.
           apply complete_shift in H22.
           apply one_step_less with (n := n0) in H22; trivial.
         ...
         * (* third case *)
           ...
           apply first_step_gamma with (n := n0) in H2; trivial.
           ...
           assert (H10': In (phi_l => phi_r) Semantics.S); trivial.
           apply H6 in H10.
           destruct H10 as (F' & HS' & E).
           assert (H10 : TS S'0 gamma gamma').
           {
             destruct F'.
             unfold TS.
           ...
           simpl in H13.
           destruct H13 as (n1 & j & gamma_j & Hj & H14 & H15 & H16).
           left.
           exists (Datatypes.S n0), (j + 1), gamma_j.
           split.
           rewrite one_step_less with
           (tau := tau) (n := n0) in Hj; trivial.
           omega.
           repeat split; trivial.
...
Module SkipOnce.
  ...
  Definition S := [skip_rule].

  (* GOALS *)
  Definition skip_once :=
    (pattern (cfg (seq skip skip) (list_var RestFRESH)) ,
     pattern (cfg skip (list_var RestFRESH))).
  Definition G0 := [skip_once].
  ...
  Lemma prove_steps :
    steps (Delta S G0).
  Proof.
    apply tranz with (G' := nil) (g := (SynDerML' (lhs skip_once) skip_rule) => (rhs skip_once)); try apply base.
    apply impl.
    - unfold Delta. simpl. unfold SynDerRL', SynDerML'. simpl. left. trivial.
    - unfold ValidML. intros gamma rho.       apply SatML_Implies.
      intros H.
      unfold SynDerML', skip_once, skip_rule in *.
      simpl lhs in *. simpl rhs in *. 
      apply SatML_Exists in H.
      destruct H as (rho' & H1 & (H2 & H3)).
      apply Proposition1 in H2.
      destruct H2 as (gamma0 & H2 & H2').
      simpl.
      case_eq (rho (lvar RestFRESH)); intros i H';
      rewrite H1 in H';
      simpl in H2';
      try rewrite H' in H2';
      try inversion H2'.
      + unfold not. intros. simpl in H.
        ...
        inversion h1.  inversion h2.  inversion h3.  inversion h4. contradiction.
      + unfold not. intros. simpl in H.
        destruct H as [h1 | [h2 | [h3 | [h4 | h5]]]].
        inversion h1.  inversion h2.  inversion h3.  inversion h4. contradiction.
      + unfold not. intros. simpl in H.
      ...
      + unfold not. intros. simpl in H.
        ...
        inversion h1.  inversion h2.  inversion h3.  inversion h4. contradiction.
      + simpl in H2.
        case_eq (rho' (svar St)); try intros M HM; try rewrite HM in H2; try inversion H2.
        case_eq (rho' (lvar Rest)); try intros R HR; try rewrite HR in H2; try inversion H2.
        rewrite HR in H4.
        subst gamma0.
        simpl in H3.
        rewrite HM, HR in H3.
      ...
      + unfold not. intros. simpl in H.
        destruct H as [h1 | [h2 | [h3 | [h4 | h5]]]].
        inversion h1.  inversion h2.  inversion h3.  inversion h4. contradiction.
    - simpl. unfold SynDerML', SynDerRL', skip_rule, skip_once. simpl.
      rewrite RLFormula_eq_dec_refl. reflexivity.
  Qed.
